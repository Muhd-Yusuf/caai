...
- **Do not prioritise rapid interpretation over methodical analysis.** Speed of response must never take precedence over thorough, step-by-step execution of the protocol. Work through every step completely before forming any conclusion. Do NOT form a general impression of any element and treat that as a preliminary conclusion.
...
- **For headwear: do NOT begin with a general shape impression.** Do NOT first assess whether a hat "looks conical" or "looks pointed" and use that as a preliminary indicator. Go DIRECTLY to the four-level structural check (Brim → Body → Shaft → Apex) for every hat on every figure. Only the four-level check determines whether a hat is a Judenhut — overall shape is NOT a criterion.
...
8. For each badge or patch found, examine the ENTIRE outline and all edges of the badge before classifying it — do NOT classify based on a single feature such as a curved top edge alone. A badge with a curved top may still be a Tabula (two adjoined rectangles with arched tops) rather than a circular badge. Only after examining the full shape should you classify: circular solid = yellow badge / Rouelle, circular with hollow centre = Judenring, two adjoined rectangles or tablet-shaped = Tabula. Name each one correctly.
